...
## 2025-10-10 · 真机背景渲染与分享缓存同步优化

### 背景与目标
真机测试发现：解签笺背景在页面渲染阶段丢失，但生成的好友分享链接仍能读取到缓存图片。目标是确保 UI 渲染继续展示远端背景，同时延续分享阶段使用缓存图片的性能优化。

### 核心改动
- **背景渲染链路修复**：`src/miniprogram/components/hanging-charm/hanging-charm.js` 的 `setupGlowStyle` 与 `scroll` 背景渲染改回优先使用 HTTPS 原图，避免 `wxfile://` 本地路径无法用于 `background-image` 时导致真机空白。
- **缓存策略保留**：同一组件新增 `ensureBackgroundImageCached`，在保持页面展示远端地址的同时，异步缓存背景图并在分享/拼接时复用本地路径，加速分享链接生成。
- **分享接口增强**：`getShareImageAsync` 支持 Promise 返回，本地页面 `onShareAppMessage`（首页、详情页）统一兼容异步结果；组件缺失、缓存失败时均回落到原有同步方案，保证分享成功率。
- **WXML 绑定优化**：`src/miniprogram/components/hanging-charm/hanging-charm.wxml` 背景层绑定顺序调整，优先展示原始背景，其次兜底缓存路径与旧字段。

### 验证建议
1. 真机打开首页/详情页并翻面，确认解签笺背景实时显示。
2. 立即点击右上角分享好友，检查分享面板缩略图加载是否无明显等待。
3. 分享链接在好友端打开，确认落地页背景与分享图一致。
...
## 2025-10-10 · 朋友圈单页模式登录体验优化

### 背景与问题
用户从朋友圈点击分享链接进入小程序时，会进入"单页模式"（场景值1154）。在这种模式下，微信禁用了 `wx.login()` 等登录相关API，导致用户点击"开始体验"按钮时出现"登录失败，请重试"的错误提示，体验不佳。

根据微信官方规范，朋友圈单页模式是为纯内容展示设计的，功能受限且无法进行用户认证操作。

### 核心改动
- **文件**: `src/miniprogram/pages/index/index.js` 
- **修改点**: `handleQuickLogin()` 方法
- **实现逻辑**:
  1. 在登录流程开始前，使用 `wx.getLaunchOptionsSync()` 检测启动场景值
  2. 当检测到场景值为 1154（朋友圈单页模式）时，不再执行登录逻辑
  3. 弹出友好提示："请点击右下角「前往小程序」以体验完整功能"
  4. 正常场景（非1154）保持原有登录流程不变

### 用户体验改进
- ❌ 修改前：朋友圈进入 → 点击"开始体验" → 显示"登录失败，请重试"（用户困惑）
- ✅ 修改后：朋友圈进入 → 点击"开始体验" → 友好提示如何进入完整小程序（引导清晰）

### 技术说明
- 场景值1154是微信官方定义的朋友圈单页模式标识
- 单页模式限制包括：无法登录、无法跳转页面、禁用多种组件和API
- 这是微信平台的硬性约束，无法通过技术手段绕过

### 验证建议
1. 在已发布的体验版/正式版中，将首页分享到朋友圈
2. 从朋友圈点击分享内容进入小程序
3. 点击"开始体验"按钮，确认出现引导提示而非登录失败错误
4. 从小程序正常入口进入首页，点击"开始体验"确认登录流程正常

**项目状态**: ✅ 朋友圈单页模式体验优化完成，待真机验证
